document.addEventListener("DOMContentLoaded", () => {
      ...
      newArray.forEach((game) => {
        if (game.id == gameID && game.download) {
          download.addEventListener("click", () => {
            window.open(
              `https://retro-bowl-15x.github.io/zips/game${gameID}.zip`,
              "_blank"
            );
          });
  ...
  setInterval(() => {
    ...
    if (realFrame && realFrame.style.display !== "none" && realFrame.contentWindow && isGameActive) {
      try { realFrame.contentWindow.focus(); } catch (e) {}
    }
  }, 1000);
